...
class LLMAgent(Agent):
    def __init__(
        ...
        if self.max_token_short:
            self.truncator = None
            try:
                _tok_name = chat_config.get('tokenization_model_name', '')
            except Exception:
                _tok_name = ''
            if isinstance(_tok_name, str) and 'gpt' not in _tok_name.lower():
                # Lazy import transformers only if a non-OpenAI tokenizer is requested
                try:
                    from transformers import AutoTokenizer  # type: ignore
                except Exception:
                    AutoTokenizer = None  # type: ignore
                self.truncator = TextTruncator(tokenization_model_name=_tok_name)
    ...
    def __update_access_counter_sub(self, cur_memory, layer_index_name, feedback):
        """Update access counters for a memory layer.
        Accepts either:
          - int index (single id), or
          - list[int], or
          - list[dict] with 'memory_index'/'id' keys.
        """
        if layer_index_name not in cur_memory:
            return
        val = cur_memory[layer_index_name]
        if val is None:
            return
        ...
        if isinstance(val, int):
            if val >= 0:
                cur_ids = [val]
        # list/tuple of ints or dicts
        elif isinstance(val, (list, tuple)):
            for item in val:
                if isinstance(item, int):
                    if item >= 0 and item not in cur_ids:
                        cur_ids.append(item)
                elif isinstance(item, dict):
                    cur_id = item.get("memory_index")
                    if cur_id is None:
                        cur_id = item.get("id") or item.get("memory_id")
                    if isinstance(cur_id, int) and cur_id >= 0 and cur_id not in cur_ids:
                        cur_ids.append(cur_id)
        else:
            # unexpected schema, ignore safely
            try:
                self.logger.warning(f"Unexpected {layer_index_name} type: {type(val)}; skipping access-counter update")
            except Exception:
                pass
        ...
        if cur_ids:
            self.brain.update_access_count_with_feed_back(
                symbol=self.trading_symbol,
                ids=cur_ids,
                feedback=feedback.get("feedback", 0.0),
            )
    ...
    def save_checkpoint(self, path: str, force: bool = False) -> None:
        ...
        agent_dir = os.path.join(path, self.agent_name)
        if os.path.exists(agent_dir):
            if force:
                shutil.rmtree(agent_dir)
            else:
                raise FileExistsError(f"Path {agent_dir} already exists")
        os.makedirs(agent_dir, exist_ok=True)
        brain_dir = os.path.join(agent_dir, "brain")
